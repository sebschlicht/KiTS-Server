...
public class KitsServer implements BroadcastReceiver.ReceiverCallback {
    ...
    public KitsServer(
        ...
        this.player = new SeriesPlayer(audioIntroDir, videoIntroDir);
    }
    ...
    public void handleRequest(DatagramPacket request) {
        ...
        if (command instanceof RegisterCommand) {
            this.broadcastReceiver.sendResponse(request.getAddress(),
                    request.getPort(), Command.SERVER_SEARCH_RESPONSE);
            System.out.println(
                    "client registered (" + request.getAddress() + ")");
        } else if (command instanceof PlayCommand) {
            PlayCommand play = (PlayCommand) command;
            System.out.println(
                    "playing: " + play.getName() + " (" + play.getType() + ")");
    ...
    public static void main(String[] args) throws IOException {
        ...
        File audioIntroDir = new File("/home/sebschlicht/Music/kits/mobile");
        File videoIntroDir = new File("/home/sebschlicht/Videos/kits/server");
        ...
        KitsServer server = new KitsServer(audioIntroDir, videoIntroDir);
